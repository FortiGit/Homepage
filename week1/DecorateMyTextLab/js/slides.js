...
function toggle() {
    ...
    if (!slides.disabled) {
        ...
        slideColl[snum].style.display= 'visible';
        fontScale();
    }
}
...
function keys(event) {
    ...
    if (event && event.keyCode && event.keyCode != null && event.keyCode == 84) {  
...
function notOperaFix() {
    ...
    outline.disabled = true;
    if (isGe) {
        slides.setAttribute('href','null');   
        slides.setAttribute('href',slideCSS); 
    }
	...
	if ($("header").offsetHeight) {
        ...
        for (var i = 0; i < slides.length; i++) {
            if (typeof(DrawingOverlay) !== "undefined") {
                new DrawingOverlay(slides[i], offsetTop);
            }
        }
        ...
        clearBtn.onclick = function() {
            var draw = document.getElementById("drawingoverlay_slide" + snum);
            draw.clear();
        ...
        toggleOverlay.onclick(); // make sure canvases are hidden
